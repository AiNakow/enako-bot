...
from src.infrastructure_plugins.qq_command_panel import (  # noqa: E402
    PANEL_REMARK,
    _sync_on_connect,
    _sync_scope,
    collect_panel_items,
...
def test_connect_hook_ignores_non_qq_bots() -> None:
    ...
    with patch(
        "src.infrastructure_plugins.qq_command_panel.synchronize_command_panels",
        synchronize,
    ):
...
def test_sync_scope_creates_panel_when_global_panel_is_missing() -> None:
    ...
    with patch("src.infrastructure_plugins.qq_command_panel._request_panel_api", request):
        result = asyncio.run(
