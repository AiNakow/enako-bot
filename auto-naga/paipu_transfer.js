...
function parsehule(h, kyoku, fan_map_) {   //tenhou log viewer requires 点, 飜) or 役満) to end strings, rest of scoring string is entirely optional
    ...
    h.fans.forEach(e => res.push(
        (JPNAME == NAMEPREF ? fan_map_[e.id].name_jp : fan_map_[e.id].name_en)
...
function generatelog(mjslog, fan_map_) {
    let log = [];
            ...
            case "RecordHule":
                {   //agari
                    ...
                    e.hules.forEach(f => {
                        ...
                        agari.push(parsehule(f, kyoku, fan_map_));
                    });
...
function parse(record, mjslog, matchmode_map_, fan_map_) {
    let res = {};
    let ruledisp = "";
    ...
    res["log"] = generatelog(mjslog, fan_map_);
    ...
    if (record.head.config.meta.mode_id) //ranked or casual
        ...
        matchmode_map_[record.head.config.meta.mode_id].room_name_jp
            : matchmode_map_[record.head.config.meta.mode_id].room_name_en;
